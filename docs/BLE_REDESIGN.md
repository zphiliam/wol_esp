...
| 配置字段 | WiFi(必填) + MQTT 服务器/端口/用户/密码(服务器必填) + WoL MAC(**可选**,终端形态由手机端管理多目标);ClientID 由芯片 ID 派生不传 |
...
| 配网 20 分钟无 BLE 连接活动 | 自动重启回正常运行(仅对已有配置设备) | — |
| 串口输入 `config`(隐藏) | 串口 CLI,产线/恢复用 | 快闪 |

> **3s 与 10s 仅是技术含义**(3s 保留配置便于改 WiFi、10s 清空),**与设备归属无关**。
> 归属在后端,采用「配网即重绑」:任一入口完成配网后,后端把 binding 覆盖为当前
> App 用户(详见 `ARCHITECTURE_v3.md` §5.1)。固件不参与归属判定。`/ble_psk.bin`
> 两种重置都不删除,二维码(mqtt_id + PSK)终身有效。

### 换网兜底:靠物理按钮,不改 `wifi_ever_ok` 逻辑(已决策)

现状:WiFi 全部连不上时,`connectWiFiMulti()` 仅在**首次连接**(`wifi_ever_ok=false`)
进 BLE 配网;**曾连过**(`wifi_ever_ok=true`)则 `ESP.restart()` 重试,不进配网。
这是为了容忍临时故障(路由器重启/停电恢复),不因网络抖动误入配网。

代价:用户**永久换路由器/改密码**时,曾连过的设备会持续重启、连不上也进不了配网。

**决策(2026-06):装盒后加物理按钮(针孔)兜底此场景,固件逻辑保持不变。**

- 按住 3s 松手即进配网,用户可重配新 WiFi —— 物理按钮覆盖了"永久换网"这一缺口。
- 重启循环中按钮仍可被捕获:`checkRuntimeTriggers()` 在 `connectWiFiMulti()` 的
  每网络等待循环内调用(`wol_esp.ino` 连接等待处),每轮有十几~几十秒轮询窗口。
- 3s 为**松手触发**,重启那刻按钮已松开,避开 GPIO9(BOOT strapping)上电进下载模式。
  交互须为「通电运行后,按住到 LED 双闪再松手」。
- 因此**不**引入"连续失败 N 次升级进配网"的软件方案(曾评估用 RTC 计数器,已放弃)。
...
- 进入连接态停广播,断开恢复
- 20 分钟无 BLE 连接活动 → 重启回正常运行（连接期间持续刷新计时器；
  无配置设备不计时，常驻配网）
