...
凭据存于设备 LittleFS `/config.json`，固件本身不含敏感信息。字段：
...
setup()
  ...
  ├─ if (!configOk)          无有效配置           → enterConfigMode()（永不返回）
  ├─ 填充 TOPIC_SUB / TOPIC_PUB
  ├─ connectWiFi()           首次超时 → enterConfigMode()；曾连接过超时 → restart()
  └─ connectMQTT()
```
...
命令：`set <key> <value>` / `show` / `save` / `ap` / `reset` / `reboot` / `help`

- `save`：写入 LittleFS 并自动重启进入正常模式
- `ap`：切换到 SoftAP 网页配置模式
- `reset`：清除 config.json 并重启
...
- **wifi_ever_ok**：记录是否曾成功连接 WiFi，存于 `/config.json`；首次连接超时进入配置模式，此后超时走重启逻辑；串口 CLI `save` 时强制重置为 `false`，确保重新配置后新 WiFi 连不上时能回到配置模式而非无限重启
